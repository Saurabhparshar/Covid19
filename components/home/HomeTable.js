...
const HomeTable = ({ data }) => {
  ...
  useEffect(() => {
    let prevStates = Object.entries(data);
    switch (sort.key) {
      case 0:
        prevStates.sort(
          (a, b) =>
        ...
        <tbody className="table-body">
          {states.map((state, index) => {
            return (
              state[0] !== "TT" && (
                <tr
                  onClick={() => history.push(`/state/${state[0]}`)}
                  key={index}
                  className="cursor-pointer group"
                >
                  <th className="max-w-[10rem] sticky left-0">
                    <div className="h-full p-2 grid place-items-center rounded-md bg-gray-200 group-hover:bg-gray-300">
